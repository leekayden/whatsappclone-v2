...
export default function Profile({ setOpen }: Props) {
  ...
  const [trackedKey, setTrackedKey] = useState(
    localStorage.getItem("triggerKey")
  );
    ...
    <div className="bg-gray-100 col-span-7 md:col-span-4 xl:col-span-5 h-screen">
      ...
      <div className="p-4">
        ...
        <div className="mt-10 px-4 shadow-md py-2 w-full bg-gray-50 flex border-2 border-green rounded-xl">
          <label htmlFor="redirectURL">Redirect URL/URI</label>
          <input
        ...
        <button
          className={`mt-8 bg-${
            isTracking ? "blue-400" : "green"
          } hover:bg-opacity-50 text-white font-medium px-3 py-2 rounded-lg cursor-pointer active:scale-95 transition`}
          onClick={handleToggleTracking}
        >
          {isTracking ? "Save Trigger" : "Set Trigger Key"}
        </button>
        {!isTracking ? (
          <p>Trigger key is currently set to "{trackedKey}".</p>
        ) : (
          <p>
            Press a trigger key on your keyboard (cannot be the "Function" key),
            then click "Save Trigger". It is currently set to "
            <strong>{trackedKey}</strong>".
          </p>
        )}
      </div>
    </div>
